...
| `0x8106-0x81D4` | `23x9` | Likely Fuel/Enrichment Lower Adjacent Candidate | No | Yes, `72 / 207` from prior MOD2 pass | Same-offset comparison only; overlaps the lower part of the broader `0x802E-0x81D4` region | Raw | Low | Prove axis and whether this is continuation, enrichment, or unrelated data. |
| `0x8A69-0x8B40` | `24x9` | Likely Spark Advance High Octane / Default | Yes | Yes | Same offset not authoritative | `raw / 2` degrees | High working label | Finish knock/fallback selector trace before removing likely qualifier. |
| `0x8B41-0x8C18` | `24x9` | Likely Spark Advance Low Octane / Alternate | Yes | Yes | Same offset not authoritative | `raw / 2` degrees | High working label | Finish knock/fallback selector trace before removing likely qualifier. |
| `0x8C19-0x8C30` | `1x24` | Likely WOT Spark Advance Vector | Yes | No | Xantia same offset differs strongly | `raw / 2` degrees | Medium-high | Prove WOT/bypass condition and final spark scheduling path. |
...
| `0x929E-0x92B5` | `1x24` | Code-Confirmed RPM Axis | Yes | No | Xantia comparable only as family pattern | `15000000 / raw period` | High | Confirm exact timer clock basis if live RPM logging becomes available. |
